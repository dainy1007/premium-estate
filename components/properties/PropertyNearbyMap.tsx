...
export default function PropertyNearbyMap({address,naverUrl,kakaoUrl}:Props){
 const ref=useRef<HTMLDivElement>(null),mapRef=useRef<any>(null),markersRef=useRef<any[]>([]),propertyMarkerRef=useRef<any>(null);
 ...
 const init=useCallback(()=>{
   ...
   k.load(()=>{const g=new k.services.Geocoder();g.addressSearch(address,(r:any[],s:string)=>{if(s!==k.services.Status.OK||!r[0])return;const pos=new k.LatLng(+r[0].y,+r[0].x);const map=new k.Map(ref.current!,{center:pos,level:5});mapRef.current=map;const markerContent=document.createElement("div");markerContent.innerHTML=`<div style="transform:translate(-50%,-100%);display:flex;flex-direction:column;align-items:center;white-space:nowrap"><div style="background:#0A2342;color:white;border:3px solid white;border-radius:18px;padding:7px 11px;font-size:13px;font-weight:800;box-shadow:0 3px 10px rgba(0,0,0,.28)">🏠 매물 위치</div><div style="width:0;height:0;border-left:8px solid transparent;border-right:8px solid transparent;border-top:12px solid #0A2342"></div></div>`;propertyMarkerRef.current=new k.CustomOverlay({map,position:pos,content:markerContent,yAnchor:0});setCenter(pos);setReady(true);search(active,radius,pos);});});
