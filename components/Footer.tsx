      ...
      <div className="mx-auto max-w-7xl">
        <div className="grid gap-7 lg:grid-cols-[.8fr_1.5fr]">
          <div>
            ...
            <p className="mt-2 text-sm leading-6 text-white/70">대표 하순영<br />대구광역시 달성군 유가읍 테크노공원로69 파크뷰타워 105호</p>
            <a href="tel:01077750014" className="mt-2 inline-block font-semibold text-[#C9A227]">010-7775-0014</a>
            <p className="mt-3 text-sm text-white/60">좋은 인연이 좋은 공간을 만듭니다.</p>
          </div>
          <nav aria-label="지역별 매물 바로가기">
            ...
            <div className="mt-3 flex flex-wrap gap-x-4 gap-y-2 text-sm text-white/75">
              {seoLinks.map(([label, href]) => <Link key={href} href={href} className="transition hover:text-white hover:underline">{label}</Link>)}
              <Link href="/properties" className="font-semibold text-white transition hover:text-[#C9A227]">전체 매물</Link>
            </div>
          </nav>
        </div>
        <div className="mt-6 flex flex-col gap-3 border-t border-white/10 pt-5 text-sm text-white/50 sm:flex-row sm:items-center sm:justify-between">
          <p>© {new Date().getFullYear()} 백조현대부동산중개. All rights reserved.</p>
          <div className="flex gap-4"><a href="tel:01077750014" className="text-white/75 hover:text-white">전화문의</a><Link href="/#contact" className="text-white/75 hover:text-white">문자상담</Link><Link href="/#map" className="text-white/75 hover:text-white">오시는길</Link></div>
        </div>
      </div>
    </footer>
  );
