...
export default function Map() {
  ...
  const [status, setStatus] = useState<MapStatus>(appKey ? "loading" : "missing-key");
  const [coordinates, setCoordinates] = useState<Coordinates>({ latitude: FALLBACK_LAT, longitude: FALLBACK_LNG });

  const initializeMap = useCallback(() => {
    if (initializedRef.current) return;
    const container = mapContainerRef.current;
    const kakaoMaps = window.kakao?.maps;
    if (!container || !kakaoMaps) return;

    initializedRef.current = true;
    kakaoMaps.load(() => {
      try {
        const fallbackPosition = new kakaoMaps.LatLng(FALLBACK_LAT, FALLBACK_LNG);
        const map = new kakaoMaps.Map(container, { center: fallbackPosition, level: 3 });
        ...
        geocoder.addressSearch(OFFICE_ADDRESS, (result, searchStatus) => {
          ...
          const infoWindow = new kakaoMaps.InfoWindow({
            content: '<div style="width:220px;padding:12px;text-align:center;font-size:14px;line-height:1.5;color:#0A2342"><strong>백조현대부동산중개</strong><br/><span style="font-size:12px;color:#666">파크뷰타워 105호</span></div>',
          });
  ...
  const showInteractiveMap = Boolean(appKey) && status !== "missing-key" && status !== "error";
