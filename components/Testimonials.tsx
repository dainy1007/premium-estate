    ...
    <section id="reviews" className="scroll-mt-28 border-t border-[#0A2342]/10 bg-[#F8F9FB] py-12 md:py-16">
      <div className="mx-auto max-w-6xl px-6 md:px-8">
        <motion.div initial={{ opacity: 0, y: 18 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true, amount: 0.2 }} transition={{ duration: 0.5 }} className="mx-auto max-w-2xl text-center">
          ...
          <p className="mt-4 text-sm leading-6 text-[#0A2342]/65 sm:text-base">고객님의 한마디가 백조현대부동산을 처음 찾는 분들에게 큰 도움이 됩니다. 이용하시는 서비스를 선택해 편하게 리뷰를 남겨주세요.</p>
        </motion.div>
        ...
        <div className="mx-auto mt-8 max-w-2xl rounded-2xl border border-[#C9A227]/25 bg-white px-5 py-4 text-center text-sm leading-6 text-[#0A2342]/70">
          <strong className="text-[#0A2342]">실제 이용 고객님의 솔직한 후기를 기다립니다.</strong><br />각 서비스의 로그인 상태에 따라 로그인 또는 업체 페이지가 먼저 열릴 수 있습니다.
        </div>
      </div>
    </section>
  );
