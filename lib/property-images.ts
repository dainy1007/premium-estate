...
export function getValidPropertyImages(files: File[]) {
  ...
  for (const file of files) {
    ...
    if (error) errors.push(error); else validFiles.push(file);
  }
  return { validFiles, errors };
}

function canvasToBlob(canvas: HTMLCanvasElement, type: string, quality?: number) {
  return new Promise<Blob>((resolve, reject) => canvas.toBlob((blob) => blob ? resolve(blob) : reject(new Error("이미지 변환에 실패했습니다.")), type, quality));
}
...
function drawWatermarkImage(ctx: CanvasRenderingContext2D, image: HTMLImageElement, width: number, height: number, scale: number, alpha: number, mode: "center" | "corner") {
  ...
  if (mode === "corner") {
    x = Math.max(CORNER_MARGIN, width - targetWidth - CORNER_MARGIN);
    y = Math.max(CORNER_MARGIN, height - targetHeight - CORNER_MARGIN);
  }
  ctx.save();
...
async function preparePropertyImage(file: File) {
  ...
  const [centerWatermark, cornerWatermark] = await Promise.all([loadWatermark(CENTER_WATERMARK_SRC), loadWatermark(CORNER_WATERMARK_SRC)]);
