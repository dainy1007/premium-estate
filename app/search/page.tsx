...
function formatPublicAddress(value?: string | null) {
  ...
  if (townIndex >= 0) return tokens.slice(0, townIndex + 1).join(" ");
  const districtIndex = tokens.findIndex((token, index) => index > 0 && /(?:시|군|구)$/.test(token));
  if (districtIndex >= 0) return tokens.slice(0, districtIndex + 1).join(" ");
  return tokens.filter((token) => !/^\d+(?:-\d+)?$/.test(token)).slice(0, 2).join(" ") || "위치 문의";
}
