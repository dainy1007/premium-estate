...
export default function Home() {
  ...
  const stats = [{ value: "15+", label: "경력" },{ value: "1200+", label: "거래" },{ value: "98%", label: "만족도" },{ value: "24H", label: "상담" }];
  const services = [
  ...
  return (
    <main className="min-h-screen bg-white pb-20 text-[#0A2342] md:pb-0">
      ...
      <section className="relative isolate overflow-hidden bg-[#F8F5EF] pt-20">
        <div aria-hidden className="absolute inset-0 bg-cover bg-[center_44%]" style={{ backgroundImage: "url('https://images.unsplash.com/photo-1618221195710-dd6b41faaea6?auto=format&fit=crop&w=1800&q=88')" }} />
        <div className="absolute inset-0 bg-gradient-to-r from-white/98 via-white/88 to-white/24" />
        <div className="relative mx-auto w-full max-w-7xl px-6 pb-10 pt-12 md:px-8 md:pb-12 md:pt-16">
          <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.65 }} className="max-w-4xl text-left">
            <p className="mb-4 text-lg font-extrabold tracking-[0.16em] text-[#C9A227] sm:text-xl">백조현대부동산중개</p>
            <h1 className="text-4xl font-extrabold leading-[1.12] text-[#0A2342] sm:text-5xl lg:text-[3.45rem]">현풍·유가·구지,<br />원하는 매물을 빠르게 찾으세요</h1>
            <p className="mt-5 max-w-2xl text-base leading-7 text-[#0A2342]/70 sm:text-lg">대구 테크노폴리스 생활권의 원룸·투룸·상가·공장·토지를 현장 중심으로 안내합니다.</p>
            <form action="/search" method="get" className="mt-6 grid w-full max-w-3xl gap-3 rounded-[22px] border border-[#0A2342]/10 bg-white/88 p-3 shadow-sm backdrop-blur-md sm:grid-cols-[1fr_auto]">
              <label htmlFor="home-property-search" className="sr-only">지역, 주소, 매물명 검색</label>
              <input id="home-property-search" name="q" value={searchKeyword} onChange={(event) => setSearchKeyword(event.target.value)} placeholder="예: 현풍 원룸, 유가읍 투룸, 테크노폴리스 상가" className="min-w-0 rounded-2xl border border-[#0A2342]/10 bg-white px-5 py-4 text-[#0A2342] outline-none placeholder:text-[#0A2342]/40 focus:border-[#C9A227]" />
              <button type="submit" className="rounded-2xl bg-[#C9A227] px-7 py-4 text-sm font-bold text-[#0A2342]">매물 검색</button>
            </form>
            <div className="mt-4 flex flex-wrap gap-2">{regionLinks.map((region) => <Link key={region.label} href={region.href} className="rounded-full border border-[#0A2342]/15 bg-white/80 px-4 py-2 text-sm font-semibold text-[#0A2342] shadow-sm">{region.label}</Link>)}</div>
            <div className="mt-4 flex flex-col gap-3 sm:flex-row"><Link href="/properties" className="inline-flex items-center justify-center rounded-full border border-[#0A2342]/15 bg-white/85 px-6 py-3 text-sm font-semibold text-[#0A2342] shadow-sm">상세 조건 검색</Link><a href="tel:01077750014" className="inline-flex items-center justify-center rounded-full bg-[#0A2342] px-6 py-3 text-sm font-bold text-white shadow-sm">☎ 전화 상담 010-7775-0014</a></div>
          </motion.div>

          <motion.div initial={{ opacity: 0, y: 18 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.1, duration: 0.6 }} className="mt-5 w-full rounded-[28px] border border-[#0A2342]/8 bg-white/92 p-4 shadow-xl backdrop-blur-md sm:p-5">
            <div className="mb-4 flex items-end justify-between gap-4 text-left"><div><p className="text-xl font-extrabold text-[#0A2342]">매물유형 바로가기</p></div><span className="hidden text-xs font-medium text-[#0A2342]/50 sm:block">카드를 누르면 해당 매물만 볼 수 있어요</span></div>
            <div className="grid grid-cols-2 gap-3 md:grid-cols-3 lg:grid-cols-6">{quickLinks.map((item) => <Link key={item.label} href={`/search?type=${encodeURIComponent(item.type)}`} className="group overflow-hidden rounded-2xl border border-[#0A2342]/10 bg-white text-[#0A2342] shadow-sm transition duration-300 hover:-translate-y-1 hover:border-[#C9A227] hover:shadow-lg"><div className="h-28 bg-cover bg-center transition duration-500 group-hover:scale-105 sm:h-32" style={{ backgroundImage: `url('${item.image}')` }} /><div className="flex min-h-16 items-center justify-between gap-2 px-3 py-3 sm:px-4"><span className="text-sm font-extrabold leading-tight sm:text-[15px]">{item.label}</span><span className="shrink-0 text-lg text-[#C9A227]">›</span></div></Link>)}</div>
          </motion.div>

          <motion.div initial={{ opacity: 0, y: 18 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.18, duration: 0.6 }} className="mt-5 grid w-full max-w-4xl grid-cols-2 gap-3 md:grid-cols-4">{stats.map((stat) => <div key={stat.label} className="rounded-2xl border border-[#0A2342]/8 bg-white/88 p-5 text-center shadow-sm backdrop-blur-sm"><p className="text-3xl font-semibold text-[#C9A227]">{stat.value}</p><p className="mt-2 text-sm text-[#0A2342]/70">{stat.label}</p></div>)}</motion.div>
        </div>
      </section>
