...
export default function Home() {
  ...
  return <main className="min-h-screen bg-white pb-20 text-[#0A2342] md:pb-0">
    <Header />
    <section className="relative isolate overflow-hidden bg-[#FBF9F5]"><div aria-hidden className="absolute inset-0 bg-cover bg-[center_48%]" style={{ backgroundImage: "url('https://images.unsplash.com/photo-1618221195710-dd6b41faaea6?auto=format&fit=crop&w=2000&q=92')" }} /><div className="absolute inset-0 bg-gradient-to-r from-white/98 via-white/82 to-white/18" /><div className="absolute inset-0 bg-white/10" /><div className="relative mx-auto flex min-h-[330px] w-full max-w-7xl items-center px-6 py-10 md:min-h-[360px] md:px-8 md:py-12"><motion.div initial={{ opacity: 0, y: 18 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.6 }} className="max-w-3xl text-left"><p className="mb-3 text-xl font-extrabold tracking-[0.1em] text-[#C99700] sm:text-2xl">백조현대부동산중개</p><h1 className="text-4xl font-extrabold leading-[1.12] sm:text-5xl lg:text-[3.35rem]">현풍·유가·구지,<br />원하는 매물을 빠르게 찾으세요</h1><p className="mt-5 max-w-2xl text-sm leading-7 text-[#0A2342]/70 sm:text-base">대구 테크노폴리스 생활권의 원룸·투룸·상가·공장·토지를 현장 중심으로 안내합니다.</p></motion.div></div></section>
    <section id="services" className="scroll-mt-24 bg-white py-7 md:py-9"><div className="mx-auto max-w-7xl px-4 sm:px-6 md:px-8"><div className="grid grid-cols-2 gap-3 sm:grid-cols-3 lg:grid-cols-6">{quickLinks.map((item) => <Link key={item.label} href={`/search?type=${encodeURIComponent(item.type)}`} className="group overflow-hidden rounded-2xl border border-[#0A2342]/10 bg-white shadow-sm transition hover:-translate-y-1 hover:border-[#C9A227] hover:shadow-lg"><div className="h-24 bg-cover bg-center transition duration-500 group-hover:scale-105 sm:h-28" style={{ backgroundImage: `url('${item.image}')` }} /><div className="px-3 py-3"><div className="flex items-center justify-between gap-2"><span className="text-sm font-extrabold sm:text-base">{item.label}</span><span className="text-xl text-[#C9A227]">›</span></div><p className="mt-1 text-[11px] leading-5 text-[#0A2342]/65">{item.sub}</p></div></Link>)}</div><div className="mt-4 grid grid-cols-4 gap-2 sm:gap-3">{stats.map((s) => <div key={s.label} className="rounded-xl border border-[#0A2342]/8 bg-white px-1 py-3 text-center shadow-sm sm:px-3 sm:py-4"><p className="text-xl font-extrabold text-[#C99700] sm:text-3xl">{s.value}</p><p className="mt-1 text-[9px] font-semibold leading-4 text-[#0A2342]/75 sm:text-xs">{s.label}</p></div>)}</div><Link href="/properties/map" className="group mt-5 flex items-center justify-between rounded-2xl border border-[#0A2342]/10 bg-white px-4 py-4 shadow-sm transition hover:border-[#C9A227] hover:shadow-md sm:px-5"><div className="flex min-w-0 items-center gap-3"><span className="flex h-11 w-11 shrink-0 items-center justify-center rounded-full bg-[#C9A227]/15 text-xl">⌖</span><div><p className="text-base font-extrabold sm:text-lg">지도에서 매물 찾기</p><p className="mt-0.5 text-[11px] text-[#0A2342]/60 sm:text-xs">백조현대부동산 등록 매물을 지도에서 확인하세요</p></div></div><span className="text-2xl text-[#C9A227]">›</span></Link></div></section>
    <FeaturedProperties />
    ...
    <Map /><Contact /><Testimonials /><PublicRecordsLinks /><Footer /><AiConsultButton />
  </main>;
}
