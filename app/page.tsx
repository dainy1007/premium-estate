  ...
  return (
    <main className="min-h-screen bg-white pb-20 text-[#0A2342] md:pb-0">
      ...
      <section className="relative isolate overflow-hidden bg-[#F8F5EF] pt-20">
        ...
        <div className="relative mx-auto w-full max-w-7xl px-6 pb-7 pt-7 md:px-8 md:pb-8 md:pt-8">
          <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.65 }} className="max-w-4xl text-left">
            <p className="mb-2 text-lg font-extrabold tracking-[0.16em] text-[#C9A227] sm:text-xl">백조현대부동산중개</p>
            <h1 className="text-3xl font-extrabold leading-[1.1] text-[#0A2342] sm:text-4xl lg:text-[2.85rem]">현풍·유가·구지,<br />원하는 매물을 빠르게 찾으세요</h1>
            <p className="mt-3 max-w-2xl text-sm leading-6 text-[#0A2342]/70 sm:text-base">대구 테크노폴리스 생활권의 원룸·투룸·상가·공장·토지를 현장 중심으로 안내합니다.</p>
            <form action="/search" method="get" className="mt-4 grid w-full max-w-3xl gap-2 rounded-[20px] border border-[#0A2342]/10 bg-white/88 p-2 shadow-sm backdrop-blur-md sm:grid-cols-[1fr_auto]">
              <label htmlFor="home-property-search" className="sr-only">지역, 주소, 매물명 검색</label>
              <input id="home-property-search" name="q" value={searchKeyword} onChange={(event) => setSearchKeyword(event.target.value)} placeholder="예: 현풍 원룸, 유가읍 투룸, 테크노폴리스 상가" className="min-w-0 rounded-2xl border border-[#0A2342]/10 bg-white px-5 py-3 text-[#0A2342] outline-none placeholder:text-[#0A2342]/40 focus:border-[#C9A227]" />
              <button type="submit" className="rounded-2xl bg-[#C9A227] px-7 py-3 text-sm font-bold text-[#0A2342]">매물 검색</button>
            </form>
            <div className="mt-3 flex flex-wrap gap-2">{regionLinks.map((region) => <Link key={region.label} href={region.href} className="rounded-full border border-[#0A2342]/15 bg-white/80 px-4 py-2 text-sm font-semibold text-[#0A2342] shadow-sm">{region.label}</Link>)}</div>
            <div className="mt-3 flex flex-col gap-2 sm:flex-row"><Link href="/properties" className="inline-flex items-center justify-center rounded-full border border-[#0A2342]/15 bg-white/85 px-5 py-2.5 text-sm font-semibold text-[#0A2342] shadow-sm">상세 조건 검색</Link><a href="tel:01077750014" className="inline-flex items-center justify-center rounded-full bg-[#0A2342] px-5 py-2.5 text-sm font-bold text-white shadow-sm">☎ 전화 상담 010-7775-0014</a></div>
          </motion.div>
