...
export async function GET(request: NextRequest) {
  ...
  const allRows=data??[]; const eventRows=allRows.filter(r=>String(r.path).startsWith("/__event/")); const rows=allRows.filter(r=>!String(r.path).startsWith("/__event/")); const unique=(items:typeof rows,key:"visitor_id"|"session_id")=>new Set(items.map(i=>i[key]).filter(Boolean)).size;
  ...
  const humanSessionIds=new Set(sessions.filter(s=>!s.suspectedBot).map(s=>s.sessionId)); const humanEvents=eventRows.filter(r=>humanSessionIds.has(r.session_id));
  const conversionSessions=new Set(humanEvents.map(r=>r.session_id)); const todayConversions=new Set(humanEvents.filter(r=>kstDateKey(r.created_at)===todayKey).map(r=>r.session_id)).size;
  const conversionBySession=new Map<string,string[]>(); for(const e of humanEvents){const a=conversionBySession.get(e.session_id)??[]; const label=e.path==="/__event/phone"?"전화":e.path==="/__event/kakao"?"카카오":"문의"; if(!a.includes(label))a.push(label); conversionBySession.set(e.session_id,a);}
  for(const s of sessions) (s as typeof s & {conversions?:string[]}).conversions=conversionBySession.get(s.sessionId)??[];
  ...
  const pageConversions=new Map<string,Set<string>>(); for(const s of sessions.filter(s=>!s.suspectedBot&&conversionSessions.has(s.sessionId))){for(const p of s.pages){const set=pageConversions.get(p.path)??new Set<string>();set.add(s.sessionId);pageConversions.set(p.path,set);}}
  const topPages=[...pageMap.entries()].map(([path,v])=>({path,title:pageTitle(path),views:v.views,visitors:v.visitors.size,inquiries:pageConversions.get(path)?.size??0})).sort((a,b)=>b.views-a.views).slice(0,10);
