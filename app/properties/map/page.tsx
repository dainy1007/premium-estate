...
export default function PropertyMapPage() {
  ...
  const renderMap = useCallback(() => {
    ...
    const clusterer = new km.MarkerClusterer({
      map,
      averageCenter: true,
      minLevel: 5,
      disableClickZoom: false,
      styles: [{ width: "46px", height: "46px", background: "#1683e8", borderRadius: "23px", color: "#fff", textAlign: "center", lineHeight: "46px", fontSize: "15px", fontWeight: "700", boxShadow: "0 2px 8px rgba(0,0,0,.18)" }],
    });
    const markers: unknown[] = [];
    let completed = 0;
    const total = visible.filter((p) => String(p.address || "").trim()).length;
    ...
    visible.forEach((property) => {
      ...
      geocoder.addressSearch(address, (result: Array<{ x: string; y: string }>, status: string) => {
        completed += 1;
        if (status === km.services.Status.OK && result[0]) {
          const position = new km.LatLng(Number(result[0].y), Number(result[0].x));
          const marker = new km.Marker({ position });
          const infoWindow = new km.InfoWindow({ content: `<div style="padding:8px 10px;font-size:12px;white-space:nowrap"><a href="/properties/${property.id}" style="color:#0A2342;font-weight:700;text-decoration:none">${deriveLocationFromAddress(address) || property.location || "매물"} · ${property.type || "매물"} →</a></div>` });
          km.event.addListener(marker, "click", () => infoWindow.open(map, marker));
          markers.push(marker);
        }
        if (completed === total && markers.length) clusterer.addMarkers(markers);
      });
    });
  }, [visible]);
  ...
  return <main className="min-h-screen bg-[#F8F9FB] pb-20 text-[#0A2342]">
    {appKey && <Script id="property-kakao-map-sdk" src={`https://dapi.kakao.com/v2/maps/sdk.js?appkey=${appKey}&autoload=false&libraries=services,clusterer`} strategy="afterInteractive" onLoad={initialize} onReady={initialize} />}
