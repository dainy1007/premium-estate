...
function formatCardPrice(property: Property) {
  const formatted = formatPropertyPriceDisplay(property.price);
  if (/가격\s*문의|협의|계약완료/i.test(formatted)) return formatted;
  const deal = String(property.deal_type || "").trim();
  if (deal === "매매" && !/^매매(?:가|금액)?\s*/.test(formatted)) return `매매가 ${formatted}`;
  if (deal === "전세" && !/^전세(?:가)?\s*/.test(formatted)) return `전세가 ${formatted}`;
  return formatted;
}

function publicLocation(property: Property) {
  return deriveLocationFromAddress(property.address) || String(property.location || "").trim();
}
