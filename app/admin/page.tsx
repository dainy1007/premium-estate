...
export default function AdminPage() {
  ...
  return (
    <main className="min-h-screen bg-[#F8F9FB] px-3 py-3 text-[#0A2342] sm:px-4">
      <div className="mx-auto max-w-7xl">
        <div className="flex flex-col gap-2 rounded-[22px] bg-white px-5 py-3 shadow-sm md:flex-row md:items-center md:justify-between md:px-6">
          <div>
        ...
        <section className="mt-2 grid grid-cols-2 gap-2 lg:grid-cols-4">
                  ...
                  {filtered.map((p) => {
                    ...
                    return (
                      <tr key={p.id} className="border-t align-middle text-sm">
                        ...
                        <td className="px-3 py-2.5 whitespace-nowrap">{p.type || "-"} <span className="text-[#0A2342]/50">{p.deal_type}</span></td>
                        <td className="px-3 py-2.5"><p>{p.location || "-"}</p><p className="mt-0.5 font-semibold text-[#9B7900]">{p.price || "문의"}</p></td>
                        <td className="px-3 py-2.5 whitespace-nowrap">{p.is_hidden ? "숨김" : completed ? "계약완료" : "노출중"}{p.is_featured ? " · 추천" : ""}</td>
