...
const BUILDING_USES=["단독주택","다가구주택","다세대주택","공동주택","아파트","주거용 오피스텔","업무시설","제1종 근린생활시설","제2종 근린생활시설","근린생활시설","판매시설","공장","창고시설","숙박시설","교육연구시설","자동차관련시설","기타"];
...
export default function PropertyEditPage(){
 ...
 const infoOverrideFields=([['elevator','엘리베이터'],['parking','주차'],['moveIn','입주가능일'],['heating','난방'],['approvalDate','사용승인일']] as const).filter(([key])=>!(hideElevator&&key==="elevator")&&!((isCommercial||isWarehouseFactory)&&(key==="moveIn"||key==="heating")));
 ...
 <div className="sm:col-span-2"><label className="mb-2 block font-semibold">매물명 / 제목</label><input value={form.title} onChange={e=>setForm(p=>({...p,title:e.target.value}))} className={inputClass} placeholder="매물 제목을 입력하세요"/></div>
 ...
 <div className="sm:col-span-2"><label className="mb-2 block font-semibold">지역 / 주소</label><input value={form.address} onChange={e=>setForm(p=>({...p,address:e.target.value,location:e.target.value}))} className={inputClass} placeholder="예: 달성군 현풍읍 중리 447"/><p className="mt-1 text-xs text-[#0A2342]/50">지역명 또는 전체 주소를 한 번만 입력하면 지역과 주소에 동일하게 저장됩니다.</p></div>
 ...
 <div><label className="mb-2 block font-semibold">방향</label><select value={meta.infoOverrides.direction} onChange={e=>setOverride("direction",e.target.value)} className={inputClass}><option value="">선택</option>{meta.infoOverrides.direction&&!DIRECTIONS.includes(meta.infoOverrides.direction)&&<option value={meta.infoOverrides.direction}>{meta.infoOverrides.direction}</option>}{DIRECTIONS.map(item=><option key={item} value={item}>{item}</option>)}</select></div>
 <div><label className="mb-2 block font-semibold">건축물 용도</label><select value={meta.infoOverrides.buildingUse} onChange={e=>setOverride("buildingUse",e.target.value)} className={inputClass}><option value="">선택</option>{meta.infoOverrides.buildingUse&&!BUILDING_USES.includes(meta.infoOverrides.buildingUse)&&<option value={meta.infoOverrides.buildingUse}>{meta.infoOverrides.buildingUse}</option>}{BUILDING_USES.map(item=><option key={item} value={item}>{item}</option>)}</select></div>
 </div></section>
